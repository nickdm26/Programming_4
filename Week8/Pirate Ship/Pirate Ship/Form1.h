	...
	private: System::Void timer1_Tick(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			// ADD CODE HERE
			 if ((cannonBallY + cannonBallDiameter) > Height)
			 {
...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			// ADD CODE HERE
			int tempPower = (int) tbPower->Value;


			// Read the degree of rotation from trackbar tbAim and convert to an int
			// ADD CODE HERE
			int tempAim = (int)tbAim->Value * -1;
			double radiansAngle = tempAim * 0.01745;
			
		

			// Use the power and orientation to compute the starting velocities for the cannonball
			// ADD CODE HERE
			int yDelta = Math::Cos(radiansAngle) * tempPower;
			int xDelta = Math::Sin(radiansAngle) * tempPower;
